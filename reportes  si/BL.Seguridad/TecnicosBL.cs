   ...
   public class TecnicosBL
    {
        ...
        public BindingList<Tecnico> SeleccionarTecnico (string buscar)
        {
            var query = _contexto.Tecnicos.Where(p => p.Nombre.ToLower()
                .Contains(buscar.ToLower()) == true)
                .ToList();
            var resultado = new BindingList<Tecnico>(query);
            return resultado;
